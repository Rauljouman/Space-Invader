...
function shoot(e) {
    if (!canShoot) return; // Si no se puede disparar, salir de la función
    ...
    function moveInvaderLaser() {
        ...
        if (currentLaserIndex < squares.length) {
            ...
            if (currentLaserIndex === currentShooterIndex) {
                resultDisplay.innerHTML = "GAME OVER";
                clearInterval(invadersId);
                clearInterval(invaderShootInterval); // Detener los disparos de los invaders
                return;
            }
...
function restartGame() {  //Función para poder jugar de nuevo
    clearInterval(invadersId)
    clearInterval(invaderShootInterval)
